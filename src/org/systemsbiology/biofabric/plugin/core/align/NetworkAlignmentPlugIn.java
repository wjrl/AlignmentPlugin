...
//   */
//
//  private NetAlignGraphStructure modifyNames(NetAlignGraphStructure inputStruct, UniqueLabeller idGen) {
//
//    NetAlignGraphStructure returnStruct = new NetAlignGraphStructure();
//    modifyLinkLonerSets(inputStruct.linksSmall, inputStruct.lonersSmall, idGen, NetworkAlignment.GraphType.SMALL, returnStruct);
//    modifyLinkLonerSets(inputStruct.linksLarge, inputStruct.lonersLarge, idGen, NetworkAlignment.GraphType.LARGE, returnStruct);
//    return (returnStruct);
//  }
//
//
...
//   */
//
//  private boolean modifyLinkLonerSets(ArrayList<NetLink> links, HashSet<NetNode> loneNodeIDs, UniqueLabeller idGen,
//                                      NetworkAlignment.GraphType type, NetAlignGraphStructure returnStruct) {
//
//    BuildExtractor bex = PluginSupportFactory.getBuildExtractor();
//
//    Map<NetNode, NetNode> oldToNew = new HashMap<NetNode, NetNode>();
//    Set<NetNode> nodeSet;
//
//    try {
//      nodeSet = bex.extractNodes(links, loneNodeIDs, null);
//    } catch (AsynchExitRequestException e) {
//      e.printStackTrace();
//      return (false);
//    }
//
//    // Create a map : old node -> new node with "::"
//    for (NetNode node : nodeSet) {
//
//      String newName;
...
//      }
//
//      NID newID = idGen.getNextOID();
//      NetNode newNode = PluginSupportFactory.buildNode(newID, newName);
//
//      oldToNew.put(node, newNode);
//    }
//
//    //
//    // Now create a completely new link set and lone-node-ID set
//    //
//
//    ArrayList<NetLink> newLinks = new ArrayList<NetLink>();
//    HashSet<NetNode> newLoners = new HashSet<NetNode>();
//
//    for (NetLink oldLink : links) {
//
//      NetNode newSrc = oldToNew.get(oldLink.getSrcNode()), newTrg = oldToNew.get(oldLink.getTrgNode());
//      // don't worry about if link is directed yet
//      NetLink newLink = PluginSupportFactory.buildLink(newSrc, newTrg, oldLink.getRelation(), oldLink.isShadow());
//      newLinks.add(newLink);
//    }
//
//    for (NetNode oldNode : loneNodeIDs) {
//      newLoners.add(oldToNew.get(oldNode));
//    }
//
//    if (type == NetworkAlignment.GraphType.SMALL) {
//      returnStruct.linksSmall = newLinks;
//      returnStruct.lonersSmall = newLoners;
//    } else if (type == NetworkAlignment.GraphType.LARGE) {
//      returnStruct.linksLarge = newLinks;
//      returnStruct.lonersLarge = newLoners;
//    } else {
//      throw (new IllegalArgumentException("Incorrect graph type"));
//    }
//    return (true);
